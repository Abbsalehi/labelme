...
###################################################################################################################
    ...
    def _get_segmentation_from_boxes(
        ...
        sam_model = osam.apis.get_model(
            model_name=sam_model_name,
            device="cuda" if osam.apis.is_cuda_available() else "cpu",
        )
        ...
        for box, score, label_idx in zip(boxes, scores, labels):
            ...
            # Create point prompt at box center for SAM
            center_x = (x1 + x2) // 2
            center_y = (y1 + y2) // 2
            ...
            # Get the best mask (usually the first one)
            mask = masks[0]
