...
""",
    ...
    def _get_segmentation_from_boxes(
        ...
        # Get the selected SAM model
        sam_model_name = self._selectAiModelComboBox.currentData()

        # Check if model is downloaded
        model_type = osam.apis.get_model_type_by_name(sam_model_name)
        model_type = typing.cast(type[osam.types.Model], model_type)
        if not model_type.get_size():
            if not download_ai_model(model_name=sam_model_name, parent=self):
                return []
        ...
        for box, score, label_idx in zip(boxes, scores, labels):
            ...
            try:
                ...
                extended_bbx = self._extend_bbx(bbx, extension_factor=1.05, extend_by_long_side=True)

                # Convert extended box to [x1, y1, x2, y2] format for OSAM
                ...
                # Create point prompt at box center
                center_x = float((input_box[0] + input_box[2]) / 2)
                center_y = float((input_box[1] + input_box[3]) / 2)

                # First pass: Generate mask with extended box
                mask = self._generate_mask_with_osam(
                ...
                # Uncomment the following lines for iterative refinement:
                refined_bbx = self._bbx_from_mask(mask, extend=10)
                if refined_bbx is not None:
                    x, y, w, h = refined_bbx
                    refined_box = [int(x), int(y), int(x + w), int(y + h)]
                    center_x = float((refined_box[0] + refined_box[2]) / 2)
                    center_y = float((refined_box[1] + refined_box[3]) / 2)

                    refined_mask = self._generate_mask_with_osam(
                        sam_model_name=sam_model_name,
                        image=image,
                        box=refined_box,
                        center_point=[center_x, center_y]
    ...
    def _generate_mask_with_osam(
        ...
        """Generate a single mask using OSAM API."""
        try:
            # Run SAM with box and point prompts using OSAM API
            ...
            if not response.annotations or len(response.annotations) == 0:
                return None

            # Use the first (best) annotation
            annotation = response.annotations[0]

            # Get the mask from annotation (it's a boolean numpy array)
            ...
            logger.warning(f"Error in _generate_mask_with_osam: {e}")
            return None
    ...
    def _extend_bbx(
            ...
            extend_by_long_side: bool = True
    ) -> list[float]:
        """
        Extend a bounding box by a given factor.

        Args:
            bbx: Bounding box as [x, y, width, height]
            extension_factor: Factor to extend the box (e.g., 1.05 for 5% extension)
            extend_by_long_side: If True, only extend the longer side

        Returns:
            Extended bounding box as [x, y, width, height]
        """
        ...
        # Determine the longer side and scale it
        if extend_by_long_side:
            if width > height:
                # Scale width (longer side)
                width_scaled = width * extension_factor
                height_scaled = height  # Keep height unchanged
            else:
                # Scale height (longer side)
                height_scaled = height * extension_factor
                width_scaled = width  # Keep width unchanged
        else:
            # Scale both sides equally
            width_scaled = width * extension_factor
            height_scaled = height * extension_factor

        # Calculate the new top-left corner
        x_scaled = center_x - width_scaled / 2
        y_scaled = center_y - height_scaled / 2
    ...
    def _bbx_from_mask(
        ...
        Args:
            mask: A 2D boolean or binary array where the region of interest is marked as 1 or True
            extend: Number of pixels to extend the bounding box on all sides

        Returns:
            Bounding box as [x_min, y_min, width, height] or None if mask is empty
        """
        # Squeeze to remove extra dimensions
        mask = np.squeeze(mask)

        # Find the coordinates of the non-zero regions
        non_zero_coords = np.argwhere(mask)

        if non_zero_coords.size == 0:
            logger.warning("The mask contains no non-zero regions")
            return None
        ...
        x_min = max(0, x_min - extend)
        y_min = max(0, y_min - extend)
        x_max = min(mask.shape[1] - 1, x_max + extend)
        y_max = min(mask.shape[0] - 1, y_max + extend)
    ...
    def _get_segmentation_from_boxes(
        ...
        # Get the selected SAM model name
        sam_model_name = self._selectAiModelComboBox.currentData()
        ...
        for box, score, label_idx in zip(boxes, scores, labels):
            ...
            try:
                # Use osam.apis.generate to get mask
                # This is the correct API according to osam
                masks = osam.apis.generate(
                ...
                if masks is None or len(masks) == 0:
                    logger.warning(f"No mask generated for box {box}")
                    continue
                ...
                logger.error(f"SAM prediction failed for box {box}: {e}")
                continue

            # Convert mask to polygon
            contours = self._mask_to_polygons(mask)
    ...
    def _get_segmentation_from_boxes(
        ...
        for box, score, label_idx in zip(boxes, scores, labels):
            ...
            # Convert box to format expected by OSAM [x1, y1, x2, y2]
            ...
            try:
                ...
                # Get annotations from response
                if not response.annotations or len(response.annotations) == 0:
                    logger.warning(f"No annotations returned for label '{label}'")
                    continue
                ...
                # Get the mask from annotation (it's a boolean numpy array)
                if annotation.mask is None:
                    logger.warning(f"Annotation has no mask for label '{label}'")
                    continue

                # Convert boolean mask to uint8 for contour detection
                mask_uint8 = annotation.mask.astype(np.uint8)
    ...
    def _mask_to_polygons(self, mask: np.ndarray, epsilon_factor: float = 0.001) -> list[np.ndarray]:
        """Convert binary mask to polygon contours."""
        # Find contours
        contours, _ = cv2.findContours(
            mask.astype(np.uint8),
